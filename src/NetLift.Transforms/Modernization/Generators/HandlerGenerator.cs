...
public sealed class HandlerGenerator : IHandlerGenerator
{
    ...
    public string GenerateMediatorImplementation(string namespaceName, string rootNamespace)
    {
        ...
        sb.AppendLine("/// <summary>");
        sb.AppendLine("/// Simple mediator implementation that resolves handlers from DI.");
        sb.AppendLine("/// </summary>");
        ...
        sb.AppendLine($"{DoubleIndent}var handlerType = typeof(IRequestHandler<,>).MakeGenericType(requestType, typeof(TResponse));");
        sb.AppendLine($"{DoubleIndent}var handler = _serviceProvider.GetRequiredService(handlerType);");
        sb.AppendLine($"{DoubleIndent}var method = handlerType.GetMethod(\"Handle\")!;");
        sb.AppendLine($"{DoubleIndent}return await (Task<TResponse>)method.Invoke(handler, new object[] {{ request, cancellationToken }})!;");
        sb.AppendLine($"{Indent}}}");
        sb.AppendLine("}");
